...
if (totalChanges !== 11) fail(`개정법 비교 카드 수 불일치: ${totalChanges}/11`)

const statuses = {
  theory: exam37LawDifferences['real-estate-theory'].changes[0]?.status,
  civil: exam37LawDifferences['civil-law'].changes[0]?.status,
  brokerageAug: exam37LawDifferences['brokerage-law'].changes[0]?.status,
  brokerage2025: exam37LawDifferences['brokerage-law'].changes[1]?.status,
...
for (const [key, value] of Object.entries({
  theory: 'applied',
  civil: 'applied',
  brokerageAug: 'comparison',
  brokerage2025: 'applied',
  housing: 'applied',
  farmland: 'comparison',
...
for (const marker of ['.exam-law-regime', '.exam-law-diff-grid', '@media (max-width: 980px)', '@media (max-width: 620px)']) {
