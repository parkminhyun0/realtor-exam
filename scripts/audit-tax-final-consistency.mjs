...
const leafSets = new Map([
  ['p1s1', taxLawPart1Point01Leaves], ['p1s2', taxLawPart1Point02Leaves], ['p1s3', taxLawPart1Point03Leaves],
  ['p2s1', taxLawPart2Point01Leaves], ['p2s2', taxLawPart2Point02Leaves], ['p2s3', taxLawPart2Point03Leaves],
  ['p3s1', taxLawPart3Point01Leaves], ['p3s2', taxLawPart3Point02Leaves], ['p3s3', taxLawPart3Point03Leaves],
])
const layerByPoint = new Map([
  ['p1s1', 'src/tax-law-point01-leaf-layer.js'], ['p1s2', 'src/tax-law-point02-leaf-layer.js'], ['p1s3', 'src/tax-law-point03-leaf-layer.js'],
  ['p2s1', 'src/tax-law-point04-leaf-layer.js'], ['p2s2', 'src/tax-law-point05-leaf-layer.js'], ['p2s3', 'src/tax-law-point06-leaf-layer.js'],
  ['p3s1', 'src/tax-law-point07-leaf-layer.js'], ['p3s2', 'src/tax-law-point08-leaf-layer.js'], ['p3s3', 'src/tax-law-point09-leaf-layer.js'],
])
...
for (const { point } of points) {
  ...
  if (leaves.length !== point.groups.length) fail(`${point.id}: 중분류 수 불일치`)
  groupCount += leaves.length

  point.groups.forEach((tocGroup, groupIndex) => {
    const leafGroup = leaves[groupIndex]
    if (leafGroup.group !== tocGroup.title) fail(`${point.id}: 중분류 제목/순서 불일치`)
    if (leafGroup.topics.length !== tocGroup.topics.length) fail(`${point.id}|${tocGroup.title}: 소분류 수 불일치`)

    tocGroup.topics.forEach((tocTopic, topicIndex) => {
      const item = leafGroup.topics[topicIndex]
      if (item.topic !== tocTopic) fail(`${point.id}|${tocGroup.title}: TOC↔leaf 불일치 ${item.topic} != ${tocTopic}`)
      ...
      for (const field of ['core', 'exam', 'trap', 'memory']) {
        ...
        leafText.push(String(item[field]))
      }
      if (!Array.isArray(item.basis) || !item.basis.length) fail(`${key}: 법령 근거 누락`)
      for (const basis of item.basis) {
        if (!String(basis.lawName || '').trim() || !String(basis.article || '').trim()) fail(`${key}: 법령칩 누락`)
  ...
  for (const reference of content.legalBases) {
    if (!reference.lawName || !Array.isArray(reference.articles) || !reference.articles.length) fail(`${point.id}: legalBases 구조 오류`)
  }
  ...
  for (const field of ['title', 'numbers', 'exceptions', 'calculation', 'sourceRefs']) {
    ...
    if (Array.isArray(value) ? !value.length : !value) fail(`${point.id}: 시험 보강 ${field} 누락`)
  }
}

if (groupCount !== 62) fail(`전체 중분류 수 불일치: ${groupCount}/62`)
if (leafCount !== 316) fail(`전체 leaf 수 불일치: ${leafCount}/316`)
if (new Set(keys).size !== 316) fail('POINT-중분류-소분류 경로 중복')

const joined = leafText.join('\n')
const postCutoff = /2026\.(?:6\.(?:[2-9]|[12]\d|3[01])|(?:7|8|9|10|11|12)\.\d{1,2})/
if (postCutoff.test(joined)) fail(`2026.6.1 이후 날짜가 시험 leaf에 직접 노출: ${joined.match(postCutoff)?.[0]}`)
...
for (const [pointId, markers] of canonicalChecks) {
  const evidence = [
    JSON.stringify(leafSets.get(pointId)), JSON.stringify(taxLawContent[pointId]),
    JSON.stringify(taxLawExamSupplement[pointId]), JSON.stringify(taxLawRateGuide[pointId] || {}),
    read(layerByPoint.get(pointId)),
  ].join('\n')
  for (const marker of markers) if (!evidence.includes(marker)) fail(`${pointId}: 핵심 숫자/기간 누락 ${marker}`)
}
...
for (const marker of [
  ...
  'REAL ESTATE TAX LAW · 제37회 기준', '근거법령 · 제37회 기준 조문', '제37회 시험 기준 조세 법령 바로보기',
...
for (let index = 1; index <= 9; index += 1) {
  ...
  const jsMarker = `./tax-law-point${no}-leaf-layer.js`
  if (!main.includes(jsMarker)) fail(`main leaf JS import 누락: ${jsMarker}`)
}
for (const no of ['01', '03', '04', '05', '06', '07', '08', '09']) {
  const cssMarker = `./tax-law-point${no}-leaf-layer.css`
  if (!main.includes(cssMarker)) fail(`main leaf CSS import 누락: ${cssMarker}`)
}
if (!main.includes('./tax-law-point01-leaf-layer.css')) fail('POINT 02가 공유하는 leaf 기본 CSS 누락')
...
if (!Object.keys(taxLawRateGuide).length) fail('세율 가이드가 비어 있습니다.')
...
console.log('- UI 연결: 9개 POINT leaf JS · 공유/전용 CSS · 좌측 직접이동 · 법령 팝업 · 모바일 목차 확인')
