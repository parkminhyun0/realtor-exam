...
import { taxLawPart2Point03LeafCount } from '../src/data/taxLawPart2Point03Leaves.js'
import { taxLawPart3Point01LeafCount } from '../src/data/taxLawPart3Point01Leaves.js'
import { taxLawPart3Point02LeafCount } from '../src/data/taxLawPart3Point02Leaves.js'
import { taxLawPart3Point03LeafCount } from '../src/data/taxLawPart3Point03Leaves.js'
...
for (const [partIndex, part] of taxLawParts.entries()) {
  ...
  if (partGroupCount !== expected.partGroups[partIndex]) fail(`PART ${part.number} 중분류 수 불일치: ${partGroupCount}/${expected.partGroups[partIndex]}`)
  if (partTopicCount !== expected.partTopics[partIndex]) fail(`PART ${part.number} 소분류 수 불일치: ${partTopicCount}/${expected.partTopics[partIndex]}`)
...
if (taxLawPart2Point03LeafCount !== 34) fail(`PART 2 POINT 03 재산세 leaf 완료 수 불일치: ${taxLawPart2Point03LeafCount}/34`)
const part2LeafCount = taxLawPart2Point01LeafCount + taxLawPart2Point02LeafCount + taxLawPart2Point03LeafCount
if (part2LeafCount !== 119) fail(`PART 2 지방세 leaf 완료 수 불일치: ${part2LeafCount}/119`)
if (taxLawPart3Point01LeafCount !== 25) fail(`PART 3 POINT 01 종합부동산세 leaf 완료 수 불일치: ${taxLawPart3Point01LeafCount}/25`)
if (taxLawPart3Point02LeafCount !== 29) fail(`PART 3 POINT 02 종합소득세 leaf 완료 수 불일치: ${taxLawPart3Point02LeafCount}/29`)
if (taxLawPart3Point03LeafCount !== 77) fail(`PART 3 POINT 03 양도소득세 leaf 완료 수 불일치: ${taxLawPart3Point03LeafCount}/77`)
const part3LeafCount = taxLawPart3Point01LeafCount + taxLawPart3Point02LeafCount + taxLawPart3Point03LeafCount
const completedLeafCount = part1LeafCount + part2LeafCount + part3LeafCount
if (part3LeafCount !== 131) fail(`PART 3 국세 leaf 완료 수 불일치: ${part3LeafCount}/131`)
if (completedLeafCount !== expected.topics) fail(`세법 전체 leaf 완료 수 불일치: ${completedLeafCount}/${expected.topics}`)

const page = read('src/TaxLawPage.jsx')
for (const marker of ['카테고리 → 대분류 → 중분류 → 소분류', 'taxLawParts.map', 'part.points.map', 'taxLawContent[point.id]']) {
...
for (const pointId of ['p1s1', 'p1s2', 'p1s3', 'p2s1', 'p2s2', 'p2s3', 'p3s1', 'p3s2', 'p3s3']) {
...
console.log(`PART 2 지방세: ${expected.partGroups[1]} 중분류 · ${expected.partTopics[1]} 소분류 · leaf ${part2LeafCount}/${expected.partTopics[1]} 완료 (취득세 61 + 등록면허세 24 + 재산세 34)`)
console.log(`PART 3 국세: ${expected.partGroups[2]} 중분류 · ${expected.partTopics[2]} 소분류 · leaf ${part3LeafCount}/${expected.partTopics[2]} 완료 (종합부동산세 25 + 종합소득세 29 + 양도소득세 77)`)
console.log(`전체 세부 학습카드: ${completedLeafCount}/${expected.topics} · 세법 leaf 100% 완료`)
console.log('POINT 본문: 9/9 존재 · 모든 POINT 중분류→소분류 leaf 연결 완료')
