...
const sources = files
  .map((absolute) => ({ file: path.relative(root, absolute).replaceAll('\\', '/'), text: fs.readFileSync(absolute, 'utf8') }))
  .filter(({ file }) => !allowed.has(file))

const publicLawParts = Array.from({ length: 11 }, (_, index) => fs.readFileSync(path.join(root, 'public', `public-law-p${String(index + 1).padStart(2, '0')}.dat`), 'utf8')).join('').replace(/\s+/g, '')
...
for (const requiredImport of ["import './exam37-law-viewer-baseline.css'", "import './exam37-baseline-content-layer.css'", "import './exam37-baseline-content-layer.js'"]) {
...
for (const marker of ['.exam37-inline-baseline', '@media (max-width: 700px)', 'grid-template-columns: 38px minmax(0, 1fr)']) if (!css.includes(marker)) fail(`제37회 baseline 반응형 CSS 누락: ${marker}`)

const popupCss = read('src/exam37-law-viewer-baseline.css')
for (const marker of ['.law-article-popup__exam-baseline', '.law-article-text__version', '@media (max-width: 620px)']) if (!popupCss.includes(marker)) fail(`제37회 법령팝업 CSS 누락: ${marker}`)
