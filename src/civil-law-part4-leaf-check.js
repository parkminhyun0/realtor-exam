...
import { civilLawPart4Visuals } from './data/civilLawPart4Visuals.js'
...
function currentKey(page) {
  ...
  return node?.partNumber === '4' ? node.key : null
}
...
function renderCheck(visual) {
  ...
  const special = visual.special
    ? `<div class="civil-leaf-recall-check__special"><b>특별법 전환 포인트</b><p><span>민법 일반</span>${esc(visual.special.general)}</p><p><span>특별법</span>${esc(visual.special.special)}</p><small>${esc(visual.special.basis)}</small></div>`
    : ''

  return `<section class="civil-leaf-recall-check" data-civil-leaf-practice="true" data-civil-part="4" data-topic="${esc(visual.topic)}">
    <header>
      <div><small>RECALL CHECK · 세부항목 자가진단</small><h3>${esc(visual.topic)}</h3></div>
      <span>PART 4 · 31/31</span>
    </header>
...
function sync() {
  ...
  const existing = page.querySelector('[data-civil-leaf-practice="true"][data-civil-part="4"]')
