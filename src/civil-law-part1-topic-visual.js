...
import { civilLawPart1VisualByKey } from './data/civilLawPart1Visuals.js'
...
function renderVisual(node, visual) {
  ...
  return `<section class="civil-topic-visual" data-civil-part1-visual="true" data-visual-key="${esc(node.key)}" data-visual-type="${esc(visual.type)}">
    <header class="civil-topic-visual__head">
      <div>
        <small>VISUAL · PART 1 전용 다이어그램</small>
...
function sync() {
  ...
  const existing = page.querySelector('[data-civil-part1-visual="true"]')
  const visual = civilLawPart1VisualByKey.get(node.key)
