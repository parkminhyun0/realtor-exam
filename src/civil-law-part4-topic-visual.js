...
import { civilLawPart4VisualByKey } from './data/civilLawPart4Visuals.js'
...
function renderSpecialRule(visual) {
  if (!visual.special) return ''
  return `<section class="civil-topic-visual__special" aria-label="민법 일반원칙과 특별법 비교">
    <header><strong>SPECIAL RULE · 일반민법 → 특별법</strong><span>${esc(visual.special.basis)}</span></header>
    <div>
      <article><small>민법 일반</small><p>${esc(visual.special.general)}</p></article>
      <i aria-hidden="true">→</i>
      <article><small>특별법 적용</small><p>${esc(visual.special.special)}</p></article>
    </div>
  </section>`
}
...
function renderVisual(node, visual) {
  ...
  return `<section class="civil-topic-visual civil-topic-visual--part4" data-civil-part4-visual="true" data-visual-key="${esc(node.key)}" data-visual-type="${esc(visual.type)}">
    <header class="civil-topic-visual__head">
      <div>
        <small>VISUAL · PART 4 민사특별법 전용 다이어그램</small>
...
function sync() {
  ...
  const existing = page.querySelector('[data-civil-part4-visual="true"]')
  const visual = civilLawPart4VisualByKey.get(node.key)
