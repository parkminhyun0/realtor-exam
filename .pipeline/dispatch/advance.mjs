...
/* 차수별로 누가 맡고 어떤 틀을 쓰는가. 한 곳에만 적는다. */
const STEP = {
  verify: { model: 'agy-model', tpl: 'verify.txt', out: '03-review.md', prompt: 'prompt-verify.txt', label: '2차 검증' },
  review: { model: 'claude', tpl: 'review.txt', out: '03b-review.md', prompt: 'prompt-review.txt', label: '3차 리뷰' },
  impl: { model: 'codex', tpl: 'impl.txt', out: '02-impl.md', prompt: 'prompt-impl.txt', label: '1차 실행' },
};

/* 2차는 Gemini 계열이면 아무 모델이나 좋다. roster 에서 첫 Gemini 를 쓴다 —
   dispatcher 가 쿼터에 걸리면 알아서 다른 계열로 넘긴다. */
function geminiModel() {
  try {
    const roster = fs.readFileSync(path.join(HERE, 'roster.tsv'), 'utf8');
...
for (const t of allTasks()) {
  ...
  if (t.failed) {
    ...
    if (!queued.has(out)) {
      added.push(['codex', `${t.id} 1차 재실행(${t.retries + 1}) ${vars.SUBJECT} ${vars.TITLE}`, rel(p), out].join('\t'));
      queued.add(out);
    }
    notes.push(`↩ ${t.id} ${who} FAIL → 1차로 되돌림 (${t.retries + 1}/${MAX_RETRY})`);
    continue;
  ...
  const model = step.model === 'agy-model' ? geminiModel() : step.model;
  ...
  added.push([model, `${t.id} ${step.label} ${vars.SUBJECT} ${vars.TITLE}`, rel(promptPath), outRel].join('\t'));
  queued.add(outRel);
